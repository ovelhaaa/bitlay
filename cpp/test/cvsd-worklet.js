const flushDenormal = (val) => Math.abs(val) < 1e-15 ? 0 : val;
const at = (param, i) => param.length > 1 ? param[i] : param[0];
class CVSDDelayProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
    this.bufferSize = sampleRate * 5;
    this.delayBuffers = [];
    this.writePtr = 0;
    this.channels = [];
    this.lfoPhase = 0;
  ...
  static get parameterDescriptors() {
    return [
      ...
      { name: "stepSize", defaultValue: 0.01, minValue: 1e-3, maxValue: 0.2 },
      { name: "clockJitter", defaultValue: 0, minValue: 0, maxValue: 100 },
      { name: "integratorLag", defaultValue: 1, minValue: 1, maxValue: 20 },
      { name: "reconstructionCutoff", defaultValue: 4e3, minValue: 100, maxValue: 1e4 },
      { name: "integratorLeak", defaultValue: 0.99, minValue: 0.9, maxValue: 1 },
      { name: "inputDrive", defaultValue: 1, minValue: 1, maxValue: 10 },
      { name: "dynamicResponse", defaultValue: 0, minValue: 0, maxValue: 1 },
      { name: "wobbleDepth", defaultValue: 0, minValue: 0, maxValue: 20 },
      { name: "wobbleRate", defaultValue: 1, minValue: 0.1, maxValue: 10 },
      { name: "feedbackTone", defaultValue: 4e3, minValue: 100, maxValue: 2e4 },
      ...
      { name: "envAttack", defaultValue: 5e-3, minValue: 1e-3, maxValue: 0.1 },
      { name: "envRelease", defaultValue: 0.05, minValue: 0.01, maxValue: 0.5 },
      { name: "circuitType", defaultValue: 0, minValue: 0, maxValue: 1 },
      { name: "minStepSize", defaultValue: 2e-3, minValue: 1e-3, maxValue: 0.2 },
      ...
      { name: "tap1_mult", defaultValue: 0.5, minValue: 0.1, maxValue: 2 },
      { name: "tap1_mix", defaultValue: 0.8, minValue: 0, maxValue: 1 },
      { name: "tap2_mult", defaultValue: 1, minValue: 0.1, maxValue: 2 },
      { name: "tap2_mix", defaultValue: 0.6, minValue: 0, maxValue: 1 },
      { name: "tap3_mult", defaultValue: 1.5, minValue: 0.1, maxValue: 2 },
      { name: "tap3_mix", defaultValue: 0.4, minValue: 0, maxValue: 1 },
      { name: "tap4_mult", defaultValue: 2, minValue: 0.1, maxValue: 2 },
      { name: "tap4_mix", defaultValue: 0.2, minValue: 0, maxValue: 1 },
      ...
      { name: "wobbleSync", defaultValue: 0.5, minValue: 0, maxValue: 1 }
    ];
  }
  createChannelState() {
    const createTapState = () => ({
      ...
      hfEnvelope: 0,
      bitHistory: 0,
      syllabicFilterState: 0,
      circuitTypeSmoothed: 0,
      reverseTime: 0
    });
    return {
  ...
  process(inputs, outputs, parameters) {
    ...
    const stereoSpread = parameters.stereoSpread[0] / 100;
    const baseStepSize = parameters.stepSize[0];
    const clockJitter = parameters.clockJitter[0] / 100;
    ...
    const envAttCoeff = 1 - Math.exp(-1 / (envAttack * sampleRate));
    const envRelCoeff = 1 - Math.exp(-1 / (envRelease * sampleRate));
    const hfAttCoeff = 1 - Math.exp(-1 / (0.01 * sampleRate));
    const hfRelCoeff = 1 - Math.exp(-1 / (0.1 * sampleRate));
    const syllabicCoeff = 1 - Math.exp(-1 / (syllabicTime / 1e3 * sampleRate));
    ...
    const lagAlpha = 1 / integratorLag;
    const tapParams = [];
    for (let t = 0; t < 4; t++) {
      tapParams.push({
    ...
    for (let i = 0; i < numSamples; i++) {
      ...
      const feedback = isFreeze ? 1 : feedbackRaw;
      const wobbleSync = parameters.wobbleSync ? at(parameters.wobbleSync, i) : 0;
      let currentWobbleRate = wobbleRate;
      if (wobbleSync > 0) {
        const scale = 500 / Math.max(10, delayMs);
        currentWobbleRate = wobbleRate * (1 - wobbleSync + wobbleSync * scale);
        currentWobbleRate = Math.max(0.01, Math.min(25, currentWobbleRate));
      }
      this.lfoPhase += 2 * Math.PI * currentWobbleRate / sampleRate;
      ...
      for (let c = 0; c < activeChannels; c++) {
        ...
        let inSample = inputChannel ? inputChannel[i] : 0;
        if (isFreeze) inSample = 0;
        const isRightChannel = c === 1;
        const channelSpread = isRightChannel ? stereoSpread : 0;
        const channelLfo = isRightChannel ? -lfoVal : lfoVal;
        let mixedOutput = 0;
        let diagLpState = 0;
        let diagCurrentStepSize = 0;
        for (let t = 0; t < activeTaps; t++) {
          ...
          const delaySamples = Math.max(1, modDelayMs) * sampleRate / 1e3;
          ...
          if (reverseModeActive) {
            ...
            const L = Math.max(
              Math.round(5e-3 * sampleRate),
              Math.round(Math.max(5, chunkSizeMs) * sampleRate / 1e3)
            );
          ...
          const decayFactor = activeTapDecay ? 1 + (multiplier - 0.5) * 1.5 : 1;
          const tapFeedbackTone = feedbackTone / (activeTapDecay ? 1 + (multiplier - 0.5) * 1 : 1);
          const tapReconCutoff = reconstructionCutoff / (activeTapDecay ? 1 + (multiplier - 0.5) * 0.6 : 1);
          const thetaRecon = 2 * Math.PI * Math.max(50, tapReconCutoff) / sampleRate;
          const reconLpCoeff = 1 - Math.exp(-thetaRecon);
          const thetaLp = 2 * Math.PI * Math.max(50, tapFeedbackTone) / sampleRate;
          const fbLpCoeff = 1 - Math.exp(-thetaLp);
          const thetaHp = 2 * Math.PI * 150 / sampleRate;
          const hpAlpha = 1 / (1 + thetaHp);
          ...
          const crossfadeTimeMs = 30;
          const crossfadeCoeff = 1 - Math.exp(-1 / (crossfadeTimeMs / 1e3 * sampleRate));
          ...
          const hfFactor = 0.5 + 2 * tapState.hfEnvelope;
          const fbFactor = 1 - Math.min(feedback, 1) * 0.2;
          const envFactor = 1 + dynamicResponse * (tapState.envelopeState * 2);
          let discreteStepScale = coupledMode ? hfFactor * fbFactor * envFactor : 1 + dynamicResponse * (tapState.envelopeState * 2);
          let discreteStepSize = baseStepSize * discreteStepScale * decayFactor * (1 + channelSpread * 0.2);
          discreteStepSize = Math.max(1e-4, Math.min(discreteStepSize, 0.5));
          const coincidence = tapState.bitHistory === 15 || tapState.bitHistory === 0 ? 1 : 0;
          ...
          let compandedStepSize = (minStepSize * decayFactor + tapState.syllabicFilterState * (maxStepSize * decayFactor - minStepSize * decayFactor)) * (1 + channelSpread * 0.2);
          compandedStepSize = Math.max(1e-4, Math.min(compandedStepSize, 0.5));
          const currentStepSize = (1 - tapState.circuitTypeSmoothed) * discreteStepSize + tapState.circuitTypeSmoothed * compandedStepSize;
          const channelClockJitter = clockJitter * (1 + channelSpread * 0.5);
          tapState.clockPhase += 1 + (Math.random() * 2 - 1) * channelClockJitter + channelLfo * (channelClockJitter * 0.2);
          if (tapState.clockPhase >= 1) {
            tapState.clockPhase -= 1;
            const bit = encoderInput >= tapState.integrator ? 1 : -1;
            tapState.bitHistory = (tapState.bitHistory << 1 | (bit > 0 ? 1 : 0)) & 15;
            const channelLagAlpha = lagAlpha * (1 - channelSpread * 0.15);
          ...
          if (isNaN(tapState.integrator)) {
            tapState.integrator = 0;
          } else {
            if (tapState.integrator > 3) tapState.integrator = 3;
            else if (tapState.integrator < -3) tapState.integrator = -3;
          }
          const channelReconCoeff = reconLpCoeff * (1 - channelSpread * 0.2);
        ...
        if (c === 0) {
          ...
          if (diagStepSize) diagStepSize[i] = bypass >= 0.5 ? 0 : diagCurrentStepSize * 10;
        }
        if (outputChannel) {
          if (bypass >= 0.5) {
            ...
            if (isFreeze) {
              ...
              outputChannel[i] = inSample * (1 - mix) + mixedOutput * mix;
            }
          }
        }
      }
